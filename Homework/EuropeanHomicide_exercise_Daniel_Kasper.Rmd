...
```{r plot-homicides, eval = TRUE, include=TRUE}
ggplot(data = Western_Europe, mapping = aes(x=Year, y=homicides_per_100k)) + geom_line(alpha = 1, aes(color = Country)) + theme_classic()
...
```{r Distributions of homicide pr. 100k in Western Europe ,eval = TRUE, include=TRUE}
plot <- ggplot(data = Western_Europe, mapping = aes(x=Year, y=homicides_per_100k)) + geom_line(alpha = 1, aes(color = Country)) + theme_classic()
plot + facet_wrap(vars(Country), nrow = 3)
...
- How would you characterize the trend compared to the homicides above?
  - I would argue for a somewhat correlation between a rise in reign period and a drop in homicide rates throughout     western europe.


```{r Reing period of Danish rulers, eval = TRUE, include=TRUE}
install.packages("tidyverse")
library(tidyverse)

monarch_data <- read_csv("data/kings.csv")

view(monarch_data)

monarch_data$End_date <- as.numeric(as.character(monarch_data$End_date))
monarch_data$Start_date <- as.numeric(as.character(monarch_data$Start_date))
monarch_data$Yearasruler <- as.numeric(as.character(monarch_data$Yearasruler))

midyear = monarch_data$End_date - (monarch_data$End_date - monarch_data$Start_date)/2

monarch_data$midyear <- midyear

df <- monarch_data

plot2 <- ggplot(df, mapping = aes(x=midyear, y=df$Yearasruler)) + geom_point(color = "red") + geom_smooth(na.rm = FALSE)
print(plot2 + labs(y="Reign Period", x="Midyear") +ggtitle("Reign Duration Among Danish Rulers"))
...
3) Question: In <250 words articulate your answer on the basis of the data visualisations to the following question: are we more civilized today?
  ...
  - I think, based on the visualizations of homicide trends, that we, atleast in western europe, have become more      civilized than compared to centuries ago. But I would argue that is a stretch to compare reign period of rulers     with the homicide rate pr. 100k because of the sheer difference in quantity of data. There might be a slight       correlation in the vizualisation trends in both datasets, but it is important to remember that correlation is      not equal to causation in these cases. 